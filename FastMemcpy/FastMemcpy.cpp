...
#pragma GCC target("arch=x86-64-v2")
...
//__attribute__((__no_sanitize__("undefined"))) inline 
static INLINE void* memcpy_tiny_sse(void* __restrict dst, const void* __restrict src, size_t size) {
...
void* memcpy_fast_sse(void* __restrict destination, const void* __restrict source, size_t size) {
    ...
    const static size_t cachesize = 0x200000; // L2-cache size
    size_t padding;
...
}// end of !xsun_likely(size <= 128)
    memcpy_tiny_sse(dst, src, size);
